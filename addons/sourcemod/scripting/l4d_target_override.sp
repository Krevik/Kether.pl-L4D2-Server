...
bool g_bCvarAllow, g_bMapStarted, g_bLateLoad, g_bLeft4Dead2;
...
public MRESReturn ChooseVictim(int attacker, Handle hReturn)
{
	...
	for( int i = 0; i < numClients; i++ )
	{
		...
		{
			// Option "voms2" then allow attacking vomited survivors ELSE (not vomited OR not incapped)
			// Option "voms" then allow choosing team 3
			team = GetClientTeam(victim);
			if( (team == 2 && (g_iOptionVoms2[class] == 1 || (g_bPinBoomer[i] == false || g_bIncapped[i] == false)) ) ||
				(team == 3 && g_iOptionVoms[class] == 1) )
			{
				// Saferoom test
				if( !g_iOptionSafe[class] || !g_bCheckpoint[victim] )
				{
					...
					if( dist < g_fOptionRange[class] ||  g_fOptionRange[class] == 0.0 )
					{
