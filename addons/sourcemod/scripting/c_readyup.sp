...
public OnPluginStart()
{
	...
	l4d_ready_max_players = CreateConVar("l4d_ready_max_players", "16", "Maximum number of players to show on the ready-up panel.", FCVAR_PLUGIN, true, 0.0, true, MAXPLAYERS+1.0);
	...
	RegConsoleCmd("sm_r", Ready_Cmd, "Toggle your ready status");
	...
	RegConsoleCmd("sm_notready", Unready_Cmd, "Mark yourself as not ready if you have set yourself as ready");
...
{
	if (inReadyUp && GetClientTeam(client) != 1)
	{
		isPlayerReady[client] = true;
		if (CheckFullReady())
			InitiateLiveCountdown();
...
UpdatePanel()
{
	...
	if(L4D2Direct_GetVSTankToSpawnThisRound(roundNumber)){
		decl String:tankPercentage[128];
		Format(tankPercentage, sizeof(tankPercentage), "Tank: %i%%", RoundToNearest(L4D2Direct_GetVSTankFlowPercent(roundNumber)*100.0));
		menuPanel.DrawText(tankPercentage);
	}else{
		decl String:tankPercentage[128];
		Format(tankPercentage, sizeof(tankPercentage), "Tank: Static Tank Map");
		menuPanel.DrawText(tankPercentage);
	}
	
	if(L4D2Direct_GetVSWitchToSpawnThisRound(roundNumber)){
		decl String:witchPercentage[128];
		Format(witchPercentage, sizeof(witchPercentage), "Witch: %i%%", RoundToNearest(L4D2Direct_GetVSWitchFlowPercent(roundNumber)*100.0));
		menuPanel.DrawText(witchPercentage);
	}else{
		decl String:witchPercentage[128];
		Format(witchPercentage, sizeof(witchPercentage), "Witch: Static Witch Map");
		menuPanel.DrawText(witchPercentage);
	}
...
bool:CheckFullReady()
{
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		if (IsClientInGame(client))
		{
			...
			if ((IsPlayer(client) && isPlayerReady[client] && GetClientTeam(client) != 1))
			{
				readyCount++;
			}
