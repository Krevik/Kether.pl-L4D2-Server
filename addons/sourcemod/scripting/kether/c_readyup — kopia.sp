...
public void OnPluginStart()
{
	...
	l4d_ready_force_extra		= CreateConVar("l4d_ready_force_extra", "1", "Number of seconds added to the duration of live count down.", FCVAR_NOTIFY, true, 0.0);
	...
	l4d_ready_secret			= CreateConVar("l4d_ready_secret", "0", "Play something good", _, true, 0.0, true, 1.0);
	...
	l4d_ready_unbalanced_min	= CreateConVar("l4d_ready_unbalanced_min", "0", "Minimum of players in each team to allow a unbalanced start.", FCVAR_NOTIFY, true, 0.0);
...
void UpdatePanel()
{
	...
	char tankString[32];
	char witchString[32];
	int witchPercent = RoundToNearest(GetWitchFlow(0) * 100.0);
	int tankPercent = RoundToNearest(GetTankFlow(0) * 100.0);
	char witchPercentString[32] = "";
	char tankPercentString[32] = "";
	IntToString(witchPercent, witchPercentString, sizeof(witchPercentString));
	IntToString(tankPercent, tankPercentString, sizeof(tankPercentString));
	if(witchPercent == 0){
		witchString = "Static";
	}else{
		witchString = "Witch: ";
		StrCat(witchString, sizeof(witchString), witchPercentString);
		StrCat(witchString, sizeof(witchString), "%");
	}
	...
	if(tankPercent == 0){
		...
		tankString = "Tank: ";
		StrCat(tankString, sizeof(tankString), tankPercentString);
		StrCat(tankString, sizeof(tankString), "%");
	}
