...
public Action:Tank_Spawn_Event(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	else 
	{
		...
		if (StrContains(gamemode, "Easy") > -1) multiplier = 0.75;
		else if (StrContains(gamemode, "Normal") > -1) multiplier = 1.0;
		else if (StrContains(gamemode, "Hard") > -1) multiplier = 1.5;
		else multiplier = 2.0;
	}
