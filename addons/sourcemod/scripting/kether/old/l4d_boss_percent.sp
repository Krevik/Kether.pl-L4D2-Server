...
public RoundStartEvent(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (!IsDKR())
	{
		CreateTimer(5.0, SetBossEveryMapWithoutFinale);
		CreateTimer(6.0, SaveBossFlows);
	}
}
...
public Action:SetBossEveryMapWithoutFinale(Handle:timer)
{
	char[] actualMapName = "null";
	GetCurrentMap(actualMapName, 32);
	if(!L4D_IsMissionFinalMap() && !InSecondHalfOfRound() && !StrEqual("c7m1_docks",actualMapName,false)
	&& !StrEqual("c13m2_southpinestream",actualMapName,false) && !StrEqual("c13m4_cutthroatcreek",actualMapName,false)
	&& !StrEqual("dkr_m1_motel",actualMapName,false) && !StrEqual("dkr_m2_carnival",actualMapName,false)  && !StrEqual("dkr_m3_tunneloflove",actualMapName,false) 
	&& !StrEqual("dkr_m4_ferris",actualMapName,false) && !StrEqual("dkr_m5_stadium",actualMapName,false) ){
		bWitch = GetConVarBool(hCvarWitchPercent);
		bTank = GetConVarBool(hCvarTankPercent);
		new String:sBuffer[64];
		new Float:fSpawnflow = GetRandomFloat(0.35,0.8);
		iTank=fSpawnflow*100;
		L4D2Direct_SetVSTankToSpawnThisRound(0, true);
		L4D2Direct_SetVSTankToSpawnThisRound(1, true);
		L4D2Direct_SetVSTankFlowPercent(0, fSpawnflow);
		L4D2Direct_SetVSTankFlowPercent(1, fSpawnflow);
		fSpawnflow = GetRandomFloat(0.35,0.8);
		iWitch=fSpawnflow*100;
		...
		if(StrEqual("c8m1_apartment",actualMapName,false)){
			fSpawnflow = 0.00;
			iWitch=fSpawnflow*100;
			...
			fSpawnflow = GetRandomFloat(0.7,0.9);
			iTank=fSpawnflow*100;
			L4D2Direct_SetVSTankToSpawnThisRound(0, true);
			L4D2Direct_SetVSTankToSpawnThisRound(1, true);
			L4D2Direct_SetVSTankFlowPercent(0, fSpawnflow);
...
public Action:VoteBoss_Cmd(client, args)
{
	if (IsValidClient(client) && GetConVarBool(hCvarVoteEnable))
	{
		...
		if (args != 2 || InSecondHalfOfRound())
		{
			if (InSecondHalfOfRound()) CPrintToChat(client, "{blue}[{default}BossVote{blue}] {default}You can only set Spawns during Ready-up in the first round.");
			else
			{
