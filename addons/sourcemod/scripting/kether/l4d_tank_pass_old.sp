...
Handle passStartForward;
Handle passEndForward;
...
public void OnPluginStart()
{
	...
	cVar = CreateConVar("l4d_tank_pass_damage", "1", "0=Allow to pass the Tank when taking any damage, 1=Prevent to pass the Tank when taking any damage.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	cVar = CreateConVar("l4d_tank_pass_notify", "0", "0=Off, 1=Display pass command info to the Tank through chat messages.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	cVar = CreateConVar("l4d_tank_pass_confirm", "0", "0=Off, 1=Ask the player if he wants to get the Tank.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	passStartForward = CreateGlobalForward("OnTankPassStart", ET_Ignore);
    passEndForward = CreateGlobalForward("OnTankPassEnd", ET_Ignore);
}
/*
|--------------------------------------------------------------------------
| ADM MENU
...
{
    Call_StartForward(passStartForward);
    Call_Finish();
	bool hasTarget;
	Menu menu;
...
void ExecCmd(int client)
{
	if (g_sCvarCmd[0] && GetClientMenu(client) == MenuSource_Normal)
		FakeClientCommand(client, "sm_tankhud off");
}
