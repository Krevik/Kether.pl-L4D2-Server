...
public void L4D_OnEnterGhostState(int client)
{
	...
	ArrayList alreadySpawnedOrGhost = new ArrayList();
	if(shallCheckDoubles){
		for (int x = 1; x <= MaxClients; x++) {
			if(IsValidClient(x)){
				if(GetClientTeam(x) == 3){
					int tmpClass = GetEntProp(x, Prop_Send, "m_zombieClass");
					alreadySpawnedOrGhost.Push(tmpClass);
				}
			}
		}
		while(FindValueInArray(alreadySpawnedOrGhost, SI) != -1){
			SI++;
			if(IsTankInPlay() && SI == SI_Spitter){
				SI++;
			}
			if(SI > SI_Charger){
				SI = SI_Smoker;
