...
public OnPluginStart() {
	...
	RegAdminCmd("sm_give", Cmd_SM_Give, ADMFLAG_BAN|ADMFLAG_KICK , "sm_give [item_name] [item_name]");
}	
...
{	
	if (!IsValidSurvivor(client))
	{
		...
		if (!IsAdmin(client))
		{
			ReplyToCommand(client, "Command is only allowed for admins");
			return Plugin_Handled;
		}

	if (argCount < 1 && IsAdmin(client))
	{
