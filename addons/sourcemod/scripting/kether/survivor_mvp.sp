...
new                 iGotRevive[MAXPLAYERS + 1];                 // CI kills
...
stock PrintLoserz(bool:bSolo, client)
{
	...
	// tell them they sucked with Revives
    if (iTotalRevives > 0)
    {
        new mvp_Revives = findMVPRevives();
        new mvp_Revives_losers[3];
        mvp_Revives_losers[0] = findMVPRevives(mvp_Revives);                                                   // second place
        mvp_Revives_losers[1] = findMVPRevives(mvp_Revives, mvp_Revives_losers[0]);                             // third
        mvp_Revives_losers[2] = findMVPRevives(mvp_Revives, mvp_Revives_losers[0], mvp_Revives_losers[1]);       // fourth
        
        for (new i = 0; i <= 2; i++)
        {
            if (IsClientAndInGame(mvp_Revives_losers[i]) && !IsFakeClient(mvp_Revives_losers[i])) 
            {
                if (bSolo)
                {
                    if (mvp_Revives_losers[i] == client)
                    {
                        Format(tmpBuffer, sizeof(tmpBuffer), "{blue}Your Rank {green}Revives{default}: {olive}#%d {blue}({default}%d {green}helps {blue}[{default}%.0f%%{blue}])", (i + 2), iGotRevive[mvp_Revives_losers[i]], (float(iGotRevive[mvp_Revives_losers[i]]) / float(iTotalRevives)) * 100);
                        CPrintToChat(mvp_Revives_losers[i], "%s", tmpBuffer);
                    }
                }
                else
                {
...
{
    new helperid = GetEventInt(event, "userid");
    new helper = GetClientOfUserId(helperid);
    
    if (helperid && IsClientAndInGame(helper) && GetClientTeam(helper) == TEAM_SURVIVOR)
    {
		iTotalRevives++;
		iGotRevive[helper]++;
    }
...
String: GetMVPString()
{
	...
	decl String:mvp_Revives_name[64];
    ...
    // Common MVP
	   ...
	   // Revives MVP
        ...
        {
            // get name from client if connected -- if not, use sClientName array
            if (IsClientConnected(mvp_Revives))
            {
                GetClientName(mvp_Revives, tmpName, sizeof(tmpName));
                if (IsFakeClient(mvp_Revives))
                {
                    StrCat(tmpName, 64, " \x01[BOT]");
                }
            } else {
                strcopy(tmpName, 64, sClientName[mvp_Revives]);
            }
            mvp_Revives_name = tmpName;
        } else {
    ...
    else
    {
		...
		//revives
            if (mvp_Revives > 0)
            {
                Format(tmpBuffer, sizeof(tmpBuffer), "{blue}[{default}MVP{blue}] Revives: {olive}%s {blue}({default}%d {green}revives {blue}[{default}%.0f%%{blue}])\n", mvp_Revives_name, iGotRevive[mvp_Revives], (float(iGotRevive[mvp_Revives]) / float(iTotalRevives)) * 100);
                StrCat(printBuffer, sizeof(printBuffer), tmpBuffer);
            }
    }
...
findMVPRevives(excludeMeA = 0, excludeMeB = 0, excludeMeC = 0)
{
    new i, maxIndex = 0;
    for(i = 1; i < sizeof(iGotRevive); i++)
    {
        if(iGotRevive[i] > iGotRevive[maxIndex] && i != excludeMeA && i != excludeMeB && i != excludeMeC)
            maxIndex = i;
    }
    return maxIndex;
}
