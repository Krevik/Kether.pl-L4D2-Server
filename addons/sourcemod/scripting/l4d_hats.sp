...
public void OnPluginStart()
{
	...
	g_hCvarView = CreateConVar(			"l4d_hats_view",		"1",			"0=Off, 1=Make a players hat visible by default when they join.", CVAR_FLAGS, true, 0.0, true, 1.0 );
	...
	RegAdminCmd("sm_hatdel",		CmdHatRemove,	ADMFLAG_ROOT,	"Allows to remove players hat");
	RegAdminCmd("sm_hatremove",		CmdHatRemove,	ADMFLAG_ROOT,	"Allows to remove players hat");
	RegAdminCmd("sm_delhat",		CmdHatRemove,	ADMFLAG_ROOT,	"Allows to remove players hat");
	RegAdminCmd("sm_removehat",		CmdHatRemove,	ADMFLAG_ROOT,	"Allows to remove players hat");
...
public Action CmdHatRemove(int client, int args)
{
	if( args == 0 || args > 1)
	{
		ReplyToCommand(client, "Usage: sm_hatremove <#userid!!!!!!!> ");
		return Plugin_Handled;
	}else{
		char sArg[32];
		GetCmdArg(1, sArg, sizeof(sArg));
