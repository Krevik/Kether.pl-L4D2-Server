...
public Plugin myinfo = 
{
	...
	author = "djromero (SkyDavid, David Romero), Harry & StarterX4",
	description = "Auto-spectate for AFK players on VS mode (translated & customized for kether)",
...
public void OnMapStart()
{
	...
	LeavedSafeRoom = true; //as ↓below↓
	afkManager_Start(); //it wasnt here, but we need it to work during ReadyUP
}
...
{
	LeavedSafeRoom = true; //was false, but as ↓below↓
	afkManager_Start(); //was _Stop, but we need it to work during ReadyUP
...
public Action afkJoinHint (Handle Timer, int client)
{
	...
	if (client && IsClientInGame(client) && afkPlayerTimeLeftWarn[client] > 0)
	{
		...
		if (GetClientTeam(client) == 1)
		{
			...
			PrintHintText(client, "You're spectating. Join any team to play.");
						...
						if (GetInfectedAttacker(i) == -1)
						{
							...
							if (afkPlayerTimeLeftWarn[i] > 0) // warn time ...
							{
								...
								if (afkPlayerTimeLeftWarn[i] <= 0)
								{
									...
									// We warn the player ....
									PrintHintText(i, "[AFK] Inactivity detected! You'll be moved to spectators in %i seconds!", afkPlayerTimeLeftAction[i]);
								}
							}
							else // player warn timeout reached ...
							{
								...
								if (afkPlayerTimeLeftAction[i] <= 0)
								{
									...
									{
										PrintHintText(i, "[AFK] Inactivity detected! You'll be moved to spectators as soon as your teammates leave the safe area.");
									}
								}
			...
			else if (afkKickEnabled)  // if player is on spectators and kick on spectators is enabled ...
			{
				...
				if (!IsClientAdmin(i))
				{
					...
					if (afkPlayerTimeLeftWarn[i] > 0) // warn time ...
					{
						...
						{
							// We warn the player ....
							PrintHintText(i, "[AFK] Inactivity detected! Join any team to play.\n Otherwise you'll be kicked from server in %i seconds.", afkPlayerTimeLeftAction[i]);
						}
					}
					else // player warn timeout reached ...
					{
						...
						if (afkPlayerTimeLeftAction[i] <=  0)
						{
							...
							{
								PrintHintText(i, "[AFK] Inactivity detected! Join any team to play.\n Otherwise you'll be kicked from server.");
							}
						}
						else // we just warn him ...
							PrintHintText(i, "[AFK] Inactivity detected! Join any team to play.\n Otherwise you'll be kicked from server in %i seconds.", afkPlayerTimeLeftAction[i]);	
...
void afkKickClient (int client)
{
	...
	PrintToChatAll("\x01\x04[TS] \x03%s \x01have been kicked from server due to inactivity.", PlayerName);
}
