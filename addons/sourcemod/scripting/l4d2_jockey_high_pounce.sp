...
public void OnPluginStart()
{
	...
	cvar_PounceScale		= CreateConVar( "l4d2_JockeyPounce_scale", "0.7", "Scale how much damage the pounce does (e.g. 0.5 will half the default damage, 5 will make it 5 times more powerfull)");
	cvar_PounceCap			= CreateConVar( "l4d2_JockeyPounce_cap", "15", "Cap of the maximum damage a pounce can do");
	cvar_PounceMinShow		= CreateConVar( "l4d2_JockeyPounce_minshow", "3", "Minimum damage a pounce should do to show the pounce message");
	cvar_PounceDisplay		= CreateConVar( "l4d2_JockeyPounce_display", "1", "How message should be shown, 0 - Disabled, 1 - Chat message, 2 - Hint Message");
...
stock DistanceJumped( int client, int victim )
{
	...
	if( damage >= GetConVarInt(cvar_PounceMinShow) )
	{
		...
		{
			if( cvar_PounceDisplay.IntValue == 1 ){
				CPrintToChatAll( "{green}★★★ {olive}%N jockey {red}high-pounced {olive}%N {default}({red}height{default}: {red}%i{default}, {red}damage{default}: {red}%i{default})", client, victim, height, damage);
			}
